...
int main(int argc, char *argv[]){
    ...
    if (rank%2==0){
      // send right
      MPI_Send(&f[nxn_loc-3], 1, MPI_DOUBLE, (rank+1)%size, tagRight, MPI_COMM_WORLD);
      MPI_Recv(f, 1, MPI_DOUBLE, (rank > 0)?(rank-1):(size-1), tagRight, MPI_COMM_WORLD, &stat);
      // send left
      MPI_Send(&f[2], 1, MPI_DOUBLE, (rank > 0)?(rank-1):(size-1), tagLeft, MPI_COMM_WORLD);
      MPI_Recv(&f[nxn_loc-1], 1, MPI_DOUBLE, (rank+1)%size, tagLeft, MPI_COMM_WORLD, &stat);
    } else {
      // send right
      MPI_Recv(f, 1, MPI_DOUBLE, (rank > 0)?(rank-1):(size-1), tagRight, MPI_COMM_WORLD, &stat);
      MPI_Send(&f[nxn_loc-3], 1, MPI_DOUBLE, (rank+1)%size, tagRight, MPI_COMM_WORLD);
